...
MANIFEST_NAME="Linux.Sdk.Manifest"
...
function install_linuxworkload() {
    DOTNET_VERSION=$1
    SOURCE=$2
    ...
    mkdir -p $SDK_MANIFESTS_DIR/linux.sdk.manifest
    cp -f $TMPDIR/unzipped/data/* $SDK_MANIFESTS_DIR/linux.sdk.manifest/

    if [ ! -f $SDK_MANIFESTS_DIR/linux.sdk.manifest/WorkloadManifest.json ]; then
    ...
    if [[ "$SOURCE" == "<auto>" ]]; then
      $DOTNET_INSTALL_DIR/dotnet workload install linux --skip-manifest-update
    else
      $DOTNET_INSTALL_DIR/dotnet workload install linux --skip-manifest-update --source $SOURCE
    fi
    ...
    echo "Done installing Linux workload $MANIFEST_VERSION"
    echo ""
}
...
if [ -z "$INSTALLED_DOTNET_SDKS" ]; then
    echo ".NET SDK version 6 or later is required to install Linux Workload."
else
    for DOTNET_SDK in $INSTALLED_DOTNET_SDKS; do
        echo "Check Linux Workload for sdk $DOTNET_SDK."
        install_linuxworkload $DOTNET_SDK $SOURCE
    done
fi

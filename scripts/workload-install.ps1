...
<#
.SYNOPSIS
Installs Linux workload manifest.
.DESCRIPTION
Installs the WorkloadManifest.json and WorkloadManifest.targets files for Linux to the dotnet sdk.
.PARAMETER Version
...
$ManifestBaseName = "Linux.Sdk.Manifest"
...
function Install-Pack([string]$Id, [string]$Version, [string]$Kind, [string]$Source) {
    ...
    switch ($Kind) {
        "manifest" {
            ...
            New-Item -Path $LinuxManifestDir -ItemType "directory" -Force | Out-Null
            Copy-Item -Path "$TempUnzipDir\data\*" -Destination $LinuxManifestDir -Force
        }
...
function Remove-Pack([string]$Id, [string]$Version, [string]$Kind) {
    switch ($Kind) {
        "manifest" {
            Remove-Item -Path $LinuxManifestDir -Recurse -Force
        }
        {($_ -eq "sdk") -or ($_ -eq "framework")} {
            $TargetDirectory = $(Join-Path -Path $DotnetInstallDir -ChildPath "packs\$Id\$Version")
...
function Install-LinuxWorkload([string]$DotnetVersion, [string]$Source)
{
    $VersionSplitSymbol = '.'
    ...
    $LinuxManifestDir = Join-Path -Path $ManifestDir -ChildPath "linux.sdk.manifest"
    $LinuxManifestFile = Join-Path -Path $LinuxManifestDir -ChildPath "WorkloadManifest.json"

    # Check and remove already installed old version.
    if (Test-Path $LinuxManifestFile) {
        $ManifestJson = $(Get-Content $LinuxManifestFile | ConvertFrom-Json)
        $OldVersion = $ManifestJson.version
        if ($OldVersion -eq $Version) {
            $DotnetWorkloadList = Invoke-Expression "& '$DotnetCommand' workload list | Select-String -Pattern '^linux'"
            if ($DotnetWorkloadList)
            {
                Write-Host "Linux Workload $Version version is already installed."
                Continue
            }
        }
    ...
    $NewManifestJson = $(Get-Content $LinuxManifestFile | ConvertFrom-Json)
    ...
    # Add linux to the installed workload metadata.
    ...
    New-Item -Path $(Join-Path -Path $DotnetInstallDir -ChildPath "metadata\workloads\$DotnetVersionBand\InstalledWorkloads\linux") -Force | Out-Null
    if (Test-Path $(Join-Path -Path $DotnetInstallDir -ChildPath "metadata\workloads\$DotnetVersionBand\InstallerType\msi")) {
        New-Item -Path "HKLM:\SOFTWARE\Microsoft\dotnet\InstalledWorkloads\Standalone\x64\$DotnetTargetVersionBand\linux" -Force | Out-Null
    }

    # Clean up
    Remove-Item -Path $TempDir -Force -Recurse

    Write-Host "Done installing Linux workload $Version"
}
...
{
    Write-Host "`n.NET SDK version 6 or later is required to install Linux Workload."
}
else
{
    ...
    {
        try {
            Write-Host "`nCheck Linux Workload for sdk $DotnetSdk"
            Install-LinuxWorkload -DotnetVersion $DotnetSdk -Source $Source
        }
        catch {
            Write-Host "Failed to install Linux Workload for sdk $DotnetSdk"
            Write-Host "$_"
            Continue
        }
    }
